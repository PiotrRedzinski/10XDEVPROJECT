...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    console.log("Generation session increment counter API called");

    // Parse and validate the input
    const body = await request.json();
    console.log("Request body:", body);

    const validationResult = incrementCounterSchema.safeParse(body);
    ...
    const { flashcardId, counterType } = validationResult.data;
    console.log("Validated data:", { flashcardId, counterType });
    ...
    // Get the generation_id for the flashcard
    ...
    console.log("Flashcard lookup result:", { flashcard, error: flashcardError });

    if (flashcardError || !flashcard || !flashcard.generation_id) {
      console.log("Flashcard not found or missing generation_id");
      return new Response(
        JSON.stringify({
          message: "Flashcard not found or doesn't have a generation_id",
        }),
        { status: 404 }
      );
    }
    ...
    console.error("Error in increment-counter endpoint:", error);
    return new Response(
      JSON.stringify({
