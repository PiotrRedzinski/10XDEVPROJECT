...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    console.log("Bulk increment counter API called");

    // Parse and validate the input
    const body = await request.json();
    console.log("Request body:", body);

    const validationResult = bulkIncrementSchema.safeParse(body);
    ...
    const { generationId, counterType, incrementBy } = validationResult.data;
    console.log("Validated data:", { generationId, counterType, incrementBy });
    ...
    console.log(
      `Updating counter: ${counterType} from ${currentValue} to ${newValue} (incrementing by ${incrementBy})`
    );
    ...
    console.error("Error in bulk-increment endpoint:", error);
    return new Response(
      JSON.stringify({
